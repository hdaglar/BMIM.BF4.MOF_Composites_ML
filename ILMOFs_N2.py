...
tpot_data = pd.read_csv('ILMOFs_N2.csv', sep=',', dtype=np.float64)
features = tpot_data.drop('N2 Uptake (mol/kg)-C-1 bar-Single', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['N2 Uptake (mol/kg)-C-1 bar-Single'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.0013531723348123132
exported_pipeline = make_pipeline(
    PolynomialFeatures(degree=2, include_bias=False, interaction_only=False),
    XGBRegressor(learning_rate=0.1, max_depth=9, min_child_weight=13, n_estimators=100, n_jobs=1, objective="reg:squarederror", subsample=0.9500000000000001, verbosity=0)
)
