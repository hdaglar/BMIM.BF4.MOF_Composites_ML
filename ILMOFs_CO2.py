...
tpot_data = pd.read_csv('ILMOFs_CO2.csv', sep=',', dtype=np.float64)
features = tpot_data.drop('CO2 Uptake (mol/kg)-C-1 bar-Single', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['CO2 Uptake (mol/kg)-C-1 bar-Single'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.29642843762881727
exported_pipeline = make_pipeline(
    StandardScaler(),
    XGBRegressor(learning_rate=0.1, max_depth=9, min_child_weight=14, n_estimators=100, n_jobs=1, objective="reg:squarederror", subsample=0.6500000000000001, verbosity=0)
)
