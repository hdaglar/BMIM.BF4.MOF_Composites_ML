...
tpot_data = pd.read_csv('PristineMOFs_CO2.csv', sep=',', dtype=np.float64)
features = tpot_data.drop('CO2 Uptake (mol/kg)-P-1 bar-Single', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['CO2 Uptake (mol/kg)-P-1 bar-Single'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.36150124171573994
exported_pipeline = make_pipeline(
    MinMaxScaler(),
    StackingEstimator(estimator=GradientBoostingRegressor(alpha=0.99, learning_rate=0.1, loss="huber", max_depth=8, max_features=0.7000000000000001, min_samples_leaf=14, min_samples_split=16, n_estimators=100, subsample=0.45)),
